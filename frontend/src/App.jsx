...
const App = () => {
  ...
  const handleSubmit = async () => {
    ...
    if (Object.keys(newErrors).length > 0) {
      addToast('error', 'Validation Error', 'Please fix the errors before submitting.');
      return;
    }

    try {
      const res = await api.post('/form', inputs, { withCredentials: true });
      ...
      addToast('error', 'Submission Failed', error.response?.data?.message || error.message);
    }
  };
      ...
      <div className="space-y-2">
        ...
        <div className="relative group">
          <input
            ...
            `}
          />
          <Icon className={`absolute left-3.5 top-1/2 transform -translate-y-1/2 w-4 h-4 transition-colors ${hasError ? 'text-red-500' : isValid ? 'text-emerald-500' : theme.textTertiary
            }`} />
          {isValid && (
  ...
  return (
    <div className={`min-h-screen transition-all duration-500 ${theme.bg} ${isDarkMode
      ...
      <div className="container mx-auto px-4 py-8 max-w-4xl">
        <div className={`${theme.cardBg} backdrop-blur-xl rounded-3xl shadow-2xl border ${isDarkMode ? 'border-gray-700/50' : 'border-white/50'
          } overflow-hidden`}>
              ...
              <div className="grid md:grid-cols-2 gap-6">
                ...
                <div className="space-y-2">
                  ...
                  <div className="relative group">
                    ...
                    <User className={`absolute left-3.5 top-1/2 transform -translate-y-1/2 w-4 h-4 ${errors.gender && touched.gender ? 'text-red-500' : touched.gender && inputs.gender && !errors.gender ? 'text-emerald-500' : theme.textTertiary
                      } transition-colors`} />
              ...
              <div className="space-y-2">
                ...
                <div className="relative group">
                  <textarea
                    ...
                    `}
                  />
                  <MapPin className={`absolute left-3.5 top-4 w-4 h-4 ${errors.address && touched.address ? 'text-red-500' : touched.address && inputs.address && !errors.address ? 'text-emerald-500' : theme.textTertiary
                    } transition-colors`} />
              ...
              <div className="space-y-2">
                ...
                <div className="relative group">
                  <input
                    ...
                    `}
                  />
                  <Lock className={`absolute left-3.5 top-1/2 transform -translate-y-1/2 w-4 h-4 ${errors.password && touched.password ? 'text-red-500' : touched.password && inputs.password && !errors.password ? 'text-emerald-500' : theme.textTertiary
                    } transition-colors`} />
                  <button
                    type="button"
